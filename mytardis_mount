...
for expRecordJson in expRecordsJson['objects']:
    ...
    expDirName = str(expRecordJson['id']) + "-" + expRecordJson['title'].encode('ascii','ignore').replace(" ","_")
    ...
    def readdir(self, path, offset):
        ...
        if len(pathComponents)==2 and pathComponents[1]!='':
            ...
            for datasetJson in datasetRecordsJson['objects']:
                ...
                datasetDirName = str(datasetJson['id']) + "-" + datasetJson['description'].encode('ascii','ignore').replace(" ","_")
